/* =========================================================================
   سرچشمه — Phase 1 · Steps 1–3 health check screen
   =========================================================================
   This is a temporary verification surface. It confirms:
   ...
   - Sample transactions seed with the right shape & count (~60–90)
...
export default function App() {
  ...
  return <HealthScreen boot={boot} digits={digits} theme={theme} onDigits={persistDigits} onTheme={persistTheme} />;
}
...
function HealthScreen({ boot, digits, theme, onDigits, onTheme }: HealthScreenProps) {
  ...
  return (
    <div className="min-h-safe px-4 py-6 max-w-2xl mx-auto space-y-4">
      {/* Header */}
      <header className="text-center pt-2 pb-2">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-3xl bg-primary/10 mb-3">
          <DropIcon />
        </div>
      ...
      <section className="card p-4 space-y-3">
        ...
        <div className="text-xs text-text-muted">
          از این تعداد، <span className="nums digits-font font-medium text-text">{digits === 'fa' ? fa(boot.sampleCount ?? 0) : boot.sampleCount}</span> رکورد نمونه‌اند (با فلگ <code className="text-[11px]">isDemo</code>) — قابل حذف انتخابی از تنظیمات.
        </div>
      </section>
...
function SegmentedControl({
  ...
  return (
    <div className="bg-surface-2 rounded-2xl p-1 flex">
      {options.map((opt) => {
